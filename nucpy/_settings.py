...
class NucpyConfig:
    ...
    def autosave(self) -> bool:
        """\
        Automatically save figures in :attr:`~nucpy._settings.NucpyConfig.figdir` (default `False`).
    ...
    def writedir(self) -> Path:
        """\
        Directory where the function nucpy.write writes to by default.
        """
    ...
    def datasetdir(self) -> Path:
        """\
        Directory for example :mod:`~nucpy.datasets` (default `'./data/'`).
        """
    ...
    def set_figure_params(
        ...
        ----------
        nucpy
            Init default values for :obj:`matplotlib.rcParams` suited for Nucpy.
        dpi
        ...
        fontsize
            Set the fontsize for several `rcParams` entries. Ignored if `nucpy=False`.
        figsize
            Set plt.rcParams['figure.figsize'].
        color_map
            Convenience method for setting the default color map. Ignored if `nucpy=False`.
        format
        ...
        if facecolor is not None:
            ...
            rcParams['axes.facecolor'] = facecolor
        if nucpy:
            from .plotting._rcmod import set_rcParams_nucpy

            set_rcParams_nucpy(fontsize=fontsize, color_map=color_map)
